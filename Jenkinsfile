...
node {
	...
	if(params.BUILD_MECHANISM == 'BUILD'){
		...
		def downloadFilePath="${env.WORKSPACE}/target/${pom_ArtifactId}-${pom_Version}.jar"
	...
	if(params.BUILD_MECHANISM == 'RELEASE'){
		...
		stage 'GetArtifactListFromNexus'			
			...
			def downloadDir = ""
			environment {
			NEXUS_CREDENTIALS = credentials('5ff62c43-8a7e-4530-aab6-b652361a4dc5')
			}
			withCredentials([usernamePassword(
			credentialsId: '5ff62c43-8a7e-4530-aab6-b652361a4dc5',
...
def UDF_ArtifactUploadToNexus(udfp_NexusBaseURL, udfp_GroupId, udfp_Version, udfp_NexusRepoName, udfp_ArtifactId, udfp_Protocol)
{
	...
	String targetZipName = "target/${udfp_ArtifactId}-${udfp_Version}.jar"
...
def UDF_DeployToCloudHub(udfp_DownloadedFilePath, udfp_PropertiesFilePath, udfp_NexusPackageURL, udfp_AppName)
{
	...
	*/
	...
	def AnypointOrganization = input(
                ...
                parameters: [
				[
					$class: 'ChoiceParameterDefinition', choices: 'WHISHWORKS\nLnD', 
	...
	def AnypointEnvironment = input(
			 ...
			 parameters: [
			 [$class: 'TextParameterDefinition', defaultValue: 'Example: LnDSandbox', description: 'Cloudhub Environment Name', name: 'DomainName']
			])
